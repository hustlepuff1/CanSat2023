...
def object_detection(cnx,modeldir,imagename,threshold=0.5,save=True):
    ...
    for image_path in images:
        ...
        if save_results:
            ...
            def starts_with_string(filepath, target_string):
                """Check if the file starts with the target string."""
                with open(filepath, 'r') as f:
                    first_line = f.readline().strip()  # strip() is used to remove any trailing whitespace or newline characters
                    return first_line == target_string
            
            with open(txt_savepath,'a') as f:
                if not starts_with_string(txt_savepath,'status variables'):
                ...
                if len(detections) > 0:
                    for detection in detections:
                        ...
                        x = height * np.tan((detection[2] - 960) * 0.001041)
                        y = height * np.tan((detection[3] - 540) * 0.001316)

                        calx = 0.980759819 * x + 0.000917887032 * y - 0.0055717019
                        caly = 0.02244757 * x + 0.89012411 * y + 0.010873488495384623

                        theta = np.radians(HEADING)  # 각도를 라디안으로 변환

                        rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])

                        # 회전 행렬을 적용하여 새로운 좌표를 계산
                        [x_rotated, y_rotated] = rotation_matrix @ [x, y]
                        [calx_rotated, caly_rotated] = rotation_matrix @ [calx, caly]
                        caly_rotated=-caly_rotated

                        # 위도 변환
                        lat_diff = caly_rotated / R
                        lat_diff_degrees = np.degrees(lat_diff)

                        # 경도 변환
                        lon_diff = calx_rotated / (R * np.cos(np.radians(lat)))  # lat is the latitude of the place where measurements are made
                        lon_diff_degrees = np.degrees(lon_diff)
