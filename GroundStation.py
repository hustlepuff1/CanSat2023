...
def Ground():
    ...
    class RealTimeGraphWindow(QMainWindow):
        def __init__(self):
            ...
            self.feedtimer.timeout.connect(self.update_feedback)
            self.feedtimer.start(100)  # update every 0.1 seconds

            self.feedback=QLabel()
            self.califeedback=QLabel()
            self.controlfeedback=QLabel()
            ...
            self.feedback.setStyleSheet(css)
            self.califeedback.setStyleSheet(css)
            self.controlfeedback.setStyleSheet(css)

            feedback_layout.addWidget(self.feedback,0,0)
            feedback_layout.addWidget(self.califeedback,1,0)
            feedback_layout.addWidget(self.controlfeedback,2,0)
        ...
        def update_image(self):          # 이미지 업데이트 method
            try:
                ...
                self.image_files = os.listdir(self.image_directory)  # self.image_files를 새로운 이미지들로 업데이트
                self.image_files = sorted(self.image_files, key=extract_number)

                # If a new image file has been added, update the current image index.
                if len(self.image_files) > self.prev_image_files_length:
                    self.current_image_index = (self.current_image_index + 1) % len(self.image_files)
                
                # Update the previous image files length to the current length.
                self.prev_image_files_length = len(self.image_files)
                ...
                if len(self.image_files) >= 1:
                    ...
                    self.image_pixmap = pixmap.scaled(self.image_label.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
                ...
                text_data_directory = 'C:/Users/82108/Desktop/CANSAT DATA/text'
                try:
                    with open(os.path.join(text_data_directory, text_data_file), 'r') as file:
                        ...
                        for line in lines:
                            items = line.split()
                            if len(items) == 10 and items[0]!="name":  # Make sure there are at least 6 items in the line
                                # Format the line as "name x = x_coord y = y_coord"
                                formatted_line = f"{items[0]} x = {items[6]} m y = {items[7]} m lon={items[8]} lat={items[9]}"
        ...
        def update_feedback(self):
            query = 'SELECT FEEDBACK, CALIFEEDBACK, CONTROLFEEDBACK FROM FEEDBACK ORDER BY Timestamp DESC LIMIT 1'
            self.cursor.execute(query)
            feed_row = self.cursor.fetchone()
            self.cnx.commit()
            try:
                if feed_row is None:
                    var_feed = None
                    ...
                    var_feed = feed_row[0]
                    var_califeed = feed_row[1]
                    var_controlfeed = feed_row[2]

                if var_feed==None:
                    self.feedback.setText("CANSAT NOT START")
                else:
                    self.feedback.setText(var_feed)
                if var_califeed==None:
                    self.califeedback.setText("NEED MAGNETOMETER CALIBRATION")
                else:
                    self.califeedback.setText(var_califeed)
                if var_controlfeed==None:
                    self.controlfeedback.setText("CONTROL NOT START")
                else:
                    self.controlfeedback.setText(var_controlfeed)
            except:
                pass
